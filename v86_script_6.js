...
(function(){
  ...
  function normalizeSettings(){
    ...
    APP.settings.loginSplash = APP.settings.loginSplash || {enabled:true,duration:3,title:'أهلاً بك في ADNOR',text:'جاهز للسحب والإيداع والألعاب',imageData:'',mode:'eachLogin'}; if(APP.settings.loginSplash.enabled===undefined)APP.settings.loginSplash.enabled=true;
  ...
  function showLoginSplash(){
    try{
      ...
      if(cfg.enabled===false && cfg.updatedAt) return;
      if(!APP.user) return;
